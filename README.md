...
无需安装 Rust 或 Bun，有 Docker 即可一键启动。
...
> - **Intel/AMD 设备**（Windows / Linux x86、Intel Mac）→ Docker 自动选择 `amd64`
> - **Apple Silicon Mac**（M1/M2/M3/M4）→ Docker 自动选择 `arm64`，原生性能，无需 Rosetta
> - **ARM 服务器**（如 AWS Graviton、树莓派）→ 同样使用 `arm64`
...
```bash
just docker-build         # 构建本地架构的 Docker 镜像
just docker-build-multi   # 构建多架构镜像 (amd64 + arm64)
just docker-push          # 构建并推送多架构镜像到 ghcr
```
