...
export default function ContestDetailPage() {
  ...
  if (loading)
    return (
      <div className="p-6 max-w-4xl mx-auto text-center py-12 text-gray-400">
        加载中...
      </div>
    );
  if (!contest)
  ...
  if (!editing)
    return (
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {error && (
          <div className="mb-4 p-3 text-sm bg-red-50 border border-red-300 text-red-700 dark:bg-red-900/30 dark:border-red-800 dark:text-red-300">
            {error}
          </div>
        )}
        ...
        <div className="mg-box-shadow p-6 mb-6">
          <div className="flex items-start justify-between mb-3">
            <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">
              {contest.name}
              <span
                className={`ml-3 px-2 py-0.5 text-xs font-medium align-middle ${contest.status === "public" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300" : "bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400"}`}
              >
          ...
          <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
            <p>
              时间：{contest.start_time} ~ {contest.end_time}
            </p>
            {contest.link && (
              <p>
                链接：
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mg-box-shadow p-6 mb-6">
          ...
          {!editProblemsReady ? (
            ...
            <div className="space-y-1.5">
              {editProblems.map((p, idx) => (
                ...
                >
                  <span className="w-6 text-center font-mono text-gray-500 dark:text-gray-400">
                    {idx + 1}
                  </span>
                  ...
                  <button
                    ...
                    disabled={idx === 0}
                    className="px-2 py-1 text-xs border border-gray-300 text-gray-600 hover:bg-gray-200 disabled:opacity-30 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700"
                  >
                    ↑
                  </button>
                  <button
        ...
        <div className="flex gap-2">
          <button
            onClick={handleSave}
            disabled={saving || !editProblemsReady}
