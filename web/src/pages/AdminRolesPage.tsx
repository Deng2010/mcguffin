...
const PERMISSION_GROUPS: { label: string; perms: string[] }[] = [
  ...
  { label: '题目管理', perms: ['submit_problem', 'view_problems', 'approve_problem'] },
  { label: '赛事管理', perms: ['manage_contests'] },
  { label: '站点管理', perms: ['manage_site', 'edit_showcase'] },
  { label: '社区管理', perms: ['view_discussions', 'manage_discussions', 'manage_tags', 'manage_posts'] },
  { label: '通知与系统', perms: ['manage_notifications', 'manage_backups', 'view_stats'] },
]

type LeafRenderer = (perm: string, label: string) => React.ReactNode

function PermissionTree({ renderLeaf, initialOpen = false }: {
  ...
  const toggle = (label: string) => setOpenGroups(p =>
      ...
      {PERMISSION_GROUPS.map(group => (
        <div key={group.label}>
          ...
          >
            ...
            <span className="text-xs text-gray-400 ml-1">({group.perms.length})</span>
          </button>
          {openGroups.includes(group.label) && (
            <div className="ml-5 space-y-0.5 pb-1">
              {group.perms.map(perm => (
                <div key={perm} className="flex items-center gap-2 px-2 py-1 text-sm text-gray-600 dark:text-gray-400">
...
export default function AdminRolesPage() {
  ...
  const roles = Object.keys(permissions).length > 0
  ...
  if (loading) return <div className="text-center py-8 text-gray-400 dark:text-gray-500">加载中...</div>
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
        展开权限分组，勾选即可设置。角色、成员组、成员的权限取 OR 关系。超级管理员不受限制。
      </p>
        ...
        <PermissionTree
          ...
          renderLeaf={(perm, label) => (
            <div className="flex flex-col gap-0.5 w-full ml-2 border-l-2 border-gray-200 dark:border-gray-700 pl-4 py-1">
              {/* Permission name */}
              <div className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">{label}</div>
              {/* Roles */}
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-gray-400 w-8 shrink-0 font-medium">角色</span>
                {roles.map(role => {
              ...
              {localGroups.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-gray-400 w-8 shrink-0 font-medium">组</span>
                  {localGroups.map(g => {
              ...
              {users.filter(u => u.id !== 'admin').length > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs text-gray-400 w-8 shrink-0 font-medium">成员</span>
                  <div className="flex flex-wrap gap-x-2 gap-y-0.5">
                    {users.filter(u => u.id !== 'admin').map(u => {
                      const checked = u.user_permissions.includes(perm)
                      return (
                        <label key={u.id} className="flex items-center gap-0.5 text-xs cursor-pointer">
                          <input
                            type="checkbox"
                            checked={checked}
                          ...
                          />
                          <span className={checked ? 'text-gray-800 dark:text-gray-100 font-medium' : 'text-gray-500 dark:text-gray-400'}>
                            {u.display_name}
                          </span>
                        </label>
                      )
