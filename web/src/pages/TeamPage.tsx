...
export default function TeamPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">成员</h1>

      {user?.team_status !== 'joined' && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200">
          <p className="text-blue-700">您还不是团队成员，<Link to="/apply" className="underline font-medium">申请加入团队</Link> 以参与协作</p>
        </div>
      )}

      {canManage && requests.length > 0 && (
        <div className="mb-8">
          <h2 className="text-lg font-semibold mb-4 text-gray-700">待处理入队申请</h2>
          <div className="space-y-2">
            {requests.map(req => (
              <div key={req.id} className="flex items-center justify-between p-4 bg-white border border-gray-300">
                <div>
                  <span className="font-medium">{req.user_name}</span>
                  <span className="text-sm text-gray-500 ml-2">{req.user_email}</span>
                  <p className="text-sm text-gray-600 mt-1">{req.reason}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleReviewRequest(req.id, 'approve')} className="px-4 py-2 bg-gray-800 text-white border border-gray-900 hover:bg-gray-700">接受</button>
                  <button onClick={() => handleReviewRequest(req.id, 'reject')} className="px-4 py-2 bg-white text-gray-700 border border-gray-300 hover:bg-gray-100">拒绝</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-2">
        {filteredMembers.length === 0 ? (
          <div className="text-center py-12 text-gray-400">暂无{activeTab === 'admin' ? '管理员' : activeTab === 'member' ? '成员' : ''}</div>
        ) : (
          filteredMembers.map(m => (
            <div key={m.id} className="flex items-center justify-between p-4 bg-white border border-gray-300">
              <div className="flex items-center gap-3">
                ...
                <div className={`w-10 h-10 bg-gray-300 rounded-full flex items-center justify-center text-gray-600 font-bold text-sm ${m.avatar_url ? 'hidden' : ''}`}>
                  {m.name.charAt(0)}
                </div>
                <div>
                  <div className="font-medium flex items-center gap-2">
                    {m.name}
                    {isCurrentUser(m) && <span className="text-xs text-gray-400">(你)</span>}
                  </div>
                  <div className="text-sm text-gray-500">加入于 {m.joined_at}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                {canManageUser(m) ? (
                  <select
                    ...
                    className="text-sm border border-gray-300 px-2 py-1 bg-white focus:outline-none"
                  >
                    <option value="member">成员</option>
                    <option value="admin">管理员</option>
                  </select>
                ) : (
                  <span className="text-sm px-3 py-1 bg-gray-100 text-gray-600 border border-gray-200">
                    {roleLabel(m.role)}
                  </span>
                )}
