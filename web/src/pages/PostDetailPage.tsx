...
import type { DiscussionTag, DiscussionEmoji, DiscussionReply, PostDetail } from '../types'
...
export default function PostDetailPage() {
  ...
  const handleDeleteReply = async (replyId: string) => {
    ...
    try {
      const res = await apiFetch<any>(`/posts/${id}/reply/${replyId}`, { method: 'DELETE' })
      if (res.success) {
        loadPost()
      } else {
        alert(res.message || '删除失败')
  ...
  if (!post) return <div className="p-6 text-center text-gray-400 dark:text-gray-500 py-12">帖子不存在</div>

  const hasStatus = post.status && ['open', 'in_progress', 'resolved', 'closed'].includes(post.status)
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      {/* Post card */}
      <div className={`bg-white border dark:bg-gray-900 p-6 mb-6 ${
        post.pinned ? 'border-yellow-400 ring-1 ring-yellow-100 dark:border-yellow-800 dark:ring-yellow-900/30' : 'border-gray-300 dark:border-gray-700'
      }`}>
        {/* Emoji + Title + Delete */}
        <div className="flex items-start gap-3 mb-3">
          {post.emoji && (
            <span className="text-4xl leading-none shrink-0 mt-0.5">{post.emoji}</span>
          )}
        ...
        <div className="flex items-center gap-3 text-xs text-gray-400 dark:text-gray-500 mb-3 ml-0">
          <span className="flex items-center gap-1.5">
            {post.author_avatar_url ? (
              <img src={post.author_avatar_url} className="w-5 h-5 rounded-full object-cover" alt="" />
            ) : (
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {allTags.map(tag => (
                ...
                >
                  ...
                  {tag.name}
                  {tag.admin_only && <span className="text-[10px] text-gray-400 ml-0.5">(管理)</span>}
                </button>
              ))}
            </div>
      ...
      <div className="bg-white border border-gray-300 dark:bg-gray-900 dark:border-gray-700 shadow p-4 relative">
        ...
        <div className="relative">
          ...
          <MentionDropdown
            ...
            onSelect={(m) => {
              setReplyContent(prev => mainMention.insertMention(prev, prev.length, m))
            }}
          />
        </div>
