...
export const pluginIncr = (pid: string, ns: string, k: string) =>
  pluginAdd(pid, ns, k, 1);
export const pluginDecr = (pid: string, ns: string, k: string) =>
  pluginAdd(pid, ns, k, -1);
...
export async function pluginSetRemove(
  ...
  const res = await pluginApi<{ removed: boolean }>(
    pluginId,
    "/data/set-remove",
    {
      method: "POST",
...
export async function pluginKeys(
  ...
  const res = await pluginApi<{ keys: string[] }>(
    pluginId,
    `/data/keys?${params}`,
  );
  return res.keys;
...
export async function pluginListFiles(
  ...
  const res = await pluginApi<{ files: string[]; count: number }>(
    pluginId,
    `/files/list${params}`,
  );
  return res.files;
...
export async function pluginUserGet(
  pluginId: string,
  userId: string,
): Promise<PluginUserInfo> {
  return pluginApi<PluginUserInfo>(
    pluginId,
    `/users/${encodeURIComponent(userId)}`,
  );
}

export async function pluginUserList(
  pluginId: string,
): Promise<{ members: PluginTeamMember[]; count: number }> {
  return pluginApi<{ members: PluginTeamMember[]; count: number }>(
    pluginId,
    "/users",
  );
}
